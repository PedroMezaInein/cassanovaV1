...
class UltimosContactados extends Component {
    state = {
        leads: {
            data: [],
            total: 0,
    ...
    getLeads = async () => {
        ...
        const { at } = this.props
        apiGet(`crm/timeline/ultimos-contactos/${leads.numPage}`, at).then((response) => {
                    ...
                    <div className="card-toolbar">
                        {
                            this.isActiveButton('prev') ?
                                <span className="btn btn-icon btn-xs btn-light-primary mr-2" onClick={this.onClickprev}>
                        ...
                        }
                        {
                            this.isActiveButton('next') ?
                                <span className="btn btn-icon btn-xs btn-light-primary" onClick={this.onClickNext}>
                    ...
                    <div className="timeline timeline-5 text-dark-50">
                        {
                            leads.data.map((contacto, key) => {
                                return (
                                    <div className="timeline-item cursor-pointer-hover" key={key} onClick={(e) => { e.preventDefault(); clickOneLead(contacto.prospecto.lead.id) }}>
...
export default UltimosContactados