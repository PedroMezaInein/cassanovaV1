...
import FormLlamada from '../../components/forms/paginaWeb/seguimiento/FormLlamada'
import FormWhatsapp from '../../components/forms/paginaWeb/seguimiento/FormWhatsapp'
import MensajePrincipal from '../../components/forms/paginaWeb/seguimiento/MensajePrincipal'
...
class LeadLlamadaSalida extends Component {
    ...
    componentDidMount() {
        ...
        if (queryString) {
            ...
            if (lead) { this.getOneLeadInfoAxios(lead) }
        }
    ...
    onSubmit = async (e) => {
        ...
        if (sendCorreoValue === 'si' || sendCorreoValue === 'no') {
            ...
            apiPutForm(`crm/update/lead/llamada-saliente/${lead.id}`, form, access_token).then(
                (response) => {
    ...
    sendRegistro = async texto => {
        ...
        const { access_token } = this.props.authUser
        const { lead } = this.state
        apiPutForm(`crm/add/registro/whatsapp/${lead.id}`, { texto: texto }, access_token).then(
            (response) => {
                Swal.close()
