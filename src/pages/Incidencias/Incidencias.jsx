...
import { printResponseErrorAlert, errorAlert, createAlert, createAlertSA2WithActionOnClose, doneAlert,waitAlert, questionAlert } from '../../functions/alert'
...
class Incidencias extends Component {
    ...
    state = {
        ...
        modal_motivo_rechazo_I1: false,
        modal_motivo_rechazo_P: false,
        modal_motivo_rechazo_P1: false,
        eventos: '',
        date: '',
        permiso: '',
        motivo_rechazo: '',
    ...
    async getPermisosModal() {
        ...
        await axios.get(URL_DEV + 'permiso/permiso', { headers: { Authorization: `Bearer ${access_token}` } }).then(
            (response) => {
                ...
                let auxLideres = []
                const { options } = this.state
                response.data.direcciones.map((direccion, key) => {
                    direccion.nombre = `${direccion.nombre} ${direccion.apellido_paterno} ${direccion.apellido_materno}`
                    auxLideres.push(direccion)
                })
                ...
                response.data.data.map((permiso) => {
                    aux.push({
                        ...
                        comentario: permiso.comentario_autoriza,
                        nombre_autoriza: permiso.nom_autoriza,
                ...
                // console.log(this.state.permisosM)
            },
            (error) => {
        ...
        await axios.get(URL_DEV + 'vacaciones/vacaciones', { headers: { Authorization: `Bearer ${access_token}` } }).then(
            (response) => {
                ...
                empleados.map((empleado) => {
                    ...
                    auxEmpleados.push(empleado)
                })

                options['empleados'] = setOptions(auxEmpleados, 'nombre', 'id')
                let aux2 = []
                let aux = []
                let mes = ''
                let dia = ''
    ...
    async editVacacionesAxios(vacacion, estatus) {
        waitAlert()
        const { access_token } = this.props.authUser
        await axios.put(`${URL_DEV}v2/rh/vacaciones/${vacacion.id_vacaciones}`, { estatus: estatus }, { headers: { Authorization: `Bearer ${access_token}` } }).then(
            (response) => {
                if (estatus === 'Aceptadas')
                    doneAlert('Vacaciones aceptadas con éxito')
            ...
            aux.push(
                {
                    ...
                    fechas: setArrayTable(
                        [
                            { name: 'Fecha inicio', text: incapacidad.fecha_inicio ? dayDMY(incapacidad.fecha_inicio) : '' },
                            { name: 'Fecha fin', text: incapacidad.fecha_fin ? dayDMY(incapacidad.fecha_fin) : '' },
                        ], '250px'
                    ),
    ...
    render() {
        ...
        return (
            <Layout active='rh'  {...this.props}>
                ...
                <Modal size={"lg"} show={modal_motivo_rechazo_I} handleClose={this.handleCloseRechazo} title="COMENTARIO BREVE" >
                    <AgregarPermisosForm
                ...
                <Modal size={"lg"} show={modal_motivo_rechazo_I1} handleClose={this.handleCloseRechazo} title="COMENTARIO BREVE" >
                    <AgregarPermisosForm
                        tipoDeFormulario='rechazarElemento'
                        disabledDates={disabledDates}
                        formeditado={formeditado}
                        ...
                        options={options}
                        // empleadoId={form.idEmpleado}
                        onSubmit={(e) => { e.preventDefault(); waitAlert(); this.editEstatusIncapacidad(id_rechazo, 'Aceptado', form.motivo_rechazo); this.clearForm(); }}
                    />
                </Modal>
                <Modal size={"lg"} show={modal_motivo_rechazo_P} handleClose={this.handleCloseRechazo} title="COMENTARIO BREVE" >
                    <AgregarPermisosForm
                        tipoDeFormulario='rechazarElemento'
                        disabledDates={disabledDates}
                        formeditado={formeditado}
                ...
                <Modal size={"lg"} show={modal_motivo_rechazo_P1} handleClose={this.handleCloseRechazo} title="COMENTARIO BREVE" >
                    <AgregarPermisosForm
                        tipoDeFormulario='rechazarElemento'
                        disabledDates={disabledDates}
                        formeditado={formeditado}
                        ...
                        options={options}
                        // empleadoId={form.idEmpleado}
                        onSubmit={(e) => { e.preventDefault(); waitAlert(); this.editEstatusPermisos(id_rechazo, 'Aceptado', form.motivo_rechazo); this.clearForm(); }}
                    />
                </Modal>
                <Modal size={"lg"} title="adjuntos" show={modal_adjuntos_permisos} handleClose={this.handleCloseAdjuntos}>
                 
                    {/* <ItemSlider
                            items={adjuntoArray} />  */}
                       { adjuntoArray.length!== 0 ? 
                          <div className="col-md-12 px-2 text-center align-self-center">
                        <ItemSlider
                            items={adjuntoArray} />   </div>
                            :  
                             <div className="col-md-12 pt-5 text-center align-self-center"> <span className='font-weight-bold text-center'>
                        ...
                        <table className="table table-head-custom table-head-bg table-vertical-center">
                            <thead>
                                <tr className="text-left">
                                    ...
                                    <th className="text-center">
                                        <span className="text-dark-75 font-size-13px">Acciones</span>
                                    </th>
                                </tr>
                            </thead>
                            {
                                            ...
                                            <tbody key={key}>
                                                <tr className="font-size-13px">
                                                    ...
                                                    <td className="py-8">
                                                        <div className="d-flex align-items-center">
                                                            <div>
                                                                <div className="mb-1">{empleado.estatus}</div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="py-8">
                                                    ...
                                                    <td className="pr-0 text-center">
                                                        <span className="btn btn-icon btn-light-success btn-sm mr-2 ml-auto" onClick={(e) => {
                                                            e.preventDefault();
                                                            this.openModalRechazarP1()
                                                            createAlertSA2WithActionOnClose('¿ESTÁS SEGURO QUE DESEAS ACEPTAR EL PERMISO?', '',
                                                                // () => this.editEstatusPermisos(empleado.id, 'Aceptado', 'No rechazado'),
                        ...
                        <table className="table table-head-custom table-head-bg table-vertical-center">
                            <thead>
                                <tr className="text-left">
                                    ...
                                    <th  className="pl-7">
                                        <span className="text-dark-75 font-size-13px">Autoriza</span>
                                    </th>
                                    <th  className="pl-7">
                                        <span className="text-dark-75 font-size-13px">Comentario</span>
                                    </th>
                            ...
                            {
                                espera.length &&  espera.map((empleado, key) => {
                                    return (
                                        <tbody key={key}>
                                            <tr className="font-size-13px">
                                                <td className="py-8">
                                                ...
                                                <td>
                                                    <span>{empleado.nom_autoriza}</span>
                                                </td>
                                                <td>
                                                    <span>{empleado.comentario_autoriza}</span>
                                                </td>
                                                <td className="text-center">
                                                    <span>{setDateTableLG(empleado.inicio)}</span>
                                                </td>
                                                <td className="text-center">
                                                    <span>{setDateTableLG(empleado.fin)}</span>
                                                </td>
                                                <td className="pr-0 text-center">
                                                    <span className="btn btn-icon btn-light-success btn-sm mr-2 ml-auto" onClick={(e) => {
                                                        e.preventDefault();
                                                        createAlert('¿ESTÁS SEGURO QUE DESEAS ACEPTAR LAS VACACIONES?', '',
                                                            () => this.editVacacionesAxios(empleado, 'Aceptadas'))
                                                    }}
                                                    >
                                                        <i className="flaticon2-check-mark icon-sm"></i>
                                                    </span>
                                                    <span className="btn btn-icon  btn-light-danger btn-sm pulse pulse-danger" onClick={(e) => {
                                                        ...
                                                        createAlert('¿ESTÁS SEGURO QUE DESEAS RECHAZAR LAS VACACIONES?', '',
                                                            () => this.editVacacionesAxios(empleado, 'Rechazadas'))
                                                    }}
                                                    >
                                                        <i className="flaticon2-cross icon-sm"></i>
                                                    </span>
