...
class AccesosForm extends Component {
            ...
            case 'edit':
                if (state) {
                    if (state.acceso) {
                        ...
                        editables.departamentos = acceso.departamentos
    ...
    async getOptionsAxios() {
        ...
        await axios.get(URL_DEV + 'accesos/options', { responseType: 'json', headers: { Accept: '*/*', 'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json;', Authorization: `Bearer ${access_token}` } }).then(
            (response) => {
                ...
                options.departamentos = setSelectOptions(departamentos, 'nombre', 'id')
                let auxArray = []
                options.usuarios.map((element)=>{
                    ...
                    if(bandera !== false)
                        form.usuarios.push({ value: bandera.id.toString(), target: bandera.name, text: bandera.name } )
                    else
                ...
                options.empresas.map((element)=>{
                    ...
                    if(bandera !== false)
                        form.empresas.push( { value: bandera.id.toString(), target: bandera.name, text: bandera.name } )
                    else
                        auxArray.push(element)
                    return false
                })
                ...
                options.departamentos.forEach((element) => {
                    let flag = editables.departamentos.find((elemento) => {
                        return elemento.id.toString() === element.value.toString()
                    })
                    if(flag)
                        form.departamentos.push({ value: flag.id.toString(), name: flag.nombre, label: flag.nombre })
                })
                this.setState({ ...this.state, options, editables, form })
    ...
    onSubmit = async() => {
        ...
        const { access_token } = this.props.authUser
        await axios.post(`${URL_DEV}v2/usuarios/accesos`, form, { responseType: 'json', headers: setSingleHeader(access_token) }).then(
            (response) => {
                Swal.close()
                const { history } = this.props
                history.push({pathname: '/usuarios/accesos'});
    ...
    onSubmitEdit = async() => {
        ...
        const { access_token } = this.props.authUser
        await axios.put(`${URL_DEV}v2/usuarios/accesos/${acceso.id}`, form, { responseType: 'json', headers: setSingleHeader(access_token) }).then(
            (response) => {
                Swal.close()
                const { history } = this.props
                history.push({pathname: '/usuarios/accesos'});
    ...
    onChangeEmpresa = empresa => {
        ...
        options.empresas.find(function (_aux) {
            if (_aux.value.toString() === empresa.toString()) auxEmpresa.push(_aux)
            else aux.push(_aux)
            return false
        })
    ...
    updateEmpresa = empresa => {
        ...
        form.empresas.map((element, key) => {
            if (empresa.value.toString() !== element.value.toString()) { aux.push(element) } 
            else { options.empresas.push(element) }
            return false
        })
        form.empresas = aux
        this.setState({ ...this.state, options, form })
    ...
    downloadPlantilla = () => {
        ...
        const url = 'https://inein-aws.s3.us-east-2.amazonaws.com/plantillas/plantilla-acceso.xlsx'
        link.href = url;
        ...
        return (
            <Layout active={'usuarios'} {...this.props}>
                <Card className="card-custom">
                    <Card.Header>
                        ...
                        <div className="card-toolbar">
                            <Button icon='' className="btn btn-light-success btn-sm mr-2" only_icon="far fa-file-pdf pr-0 mr-2"
                                text='AGREGAR ACCESO' onClick={this.openModal} />
                        </div>
                    </Card.Header>
                    <Card.Body>
                        <AccesosFormulario form = { form } formeditado = { formeditado } options = { options }  onChange = { this.onChange } 
                            onChangeAndAdd = { this.onChangeAndAdd } deleteOption = { this.deleteOption } onChangeEmpresa = { this.onChangeEmpresa } 
                            updateEmpresa = { this.updateEmpresa } onSubmit = { title === 'Editar acceso' ? this.onSubmitEdit : this.onSubmit } />
                    </Card.Body>
                </Card>
                <Modal size='lg' title={title} show={modal_add_excel} handleClose={this.handleCloseAddExcel}>
                    ...
                    <div>
                        <ItemSlider items={form.adjuntos.adjuntos.files} item='adjuntos' multiple={false}
                            handleChange={this.handleChange} accept='.xlsx, .xls, .csv' />
                    </div>
                </Modal>
            </Layout>
        )
