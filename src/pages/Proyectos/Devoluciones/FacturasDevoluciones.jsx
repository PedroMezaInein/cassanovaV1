...
export default function Factura(props) {
    ...
    const addFacturaS3 =  ( ) => { //attachFiles
        ...
        apiGet(`v1/constant/admin-proyectos`, auth).then(
            (response) => {
                ...
                Promise.all(auxPromises).then(values => { addNewFacturaAxios(values, data) }).catch(err => console.error(err))
            }, (error) => { }
        ).catch((error) => { 
            Swal.close()
            Swal.fire({
    ...
    const proccessData = (datos) => {
        ...
        // console.log(datos.devolucion)
            datos.devolucion.facturas.forEach((factura) => {
    ...
    const deleteEgresoAxios = (id) => {
        ...
        apiDelete(`v2/proyectos/compras/${data.id}/facturas/${id}`, auth).then(
            (response) => {
                Swal.fire(
                    '¡Eliminado!',
        ...
        <div>
            ...
            <div>
                <TablaGeneral
                    ...
                    url={`v1/proyectos/devoluciones/facturas/${data.id}`}
                    columnas={columns}
                    numItemsPagina={20}
