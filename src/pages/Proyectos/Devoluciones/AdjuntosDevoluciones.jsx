...
import CarruselAdjuntosDevoluciones from './CarruselAdjuntosDevoluciones'
import style from './../Compras/CarruselCompras.module.scss'
...
export default function AdjuntosCompras (props) {
    ...
    const getAdjuntos = () => {
        try {
            apiGet(`v1/proyectos/devoluciones/adjuntos/${data.id}`, authUser)
                .then(res => {
                    console.log(res)

                    let facturaExtranjera = res.data.devolucion.facturas_pdf
                    let pagos = res.data.devolucion.pagos
                    let presupuesto = res.data.devolucion.presupuestos
                    ...
                    facturaExtranjera.forEach((element) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (validate()) {
            ...
            try {
                apiPostForm(`v1/proyectos/devoluciones/${props.data.id}/adjuntos`, data, authUser)
                    .then(res => {
                        console.log('res')
                        console.log(res)
                        Swal.close()
                    ...
                    .catch(err => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
                            text: 'Algo salio terriblemente mal!',
                        })
                        console.log('err', err)
                    })
            } catch (error) {
            ...
            <>
                {
                    adjuntos && adjuntos[tab] && adjuntos[tab].length > 0 ?
                        <CarruselAdjuntosDevoluciones data={adjuntos[tab]} id={data.id} getAdjuntos={getAdjuntos} />
                        :
        ...
        <>
            <div className={classes.root}>
                ...
                >
                    ...
                    {Gastos ? null : <Tab label="presupuesto" {...a11yProps(1)} name="presupuesto" onClick={() => handleTab('presupuesto')} />}
                ...
                <TabPanel value={value} index={1}>
                    <div>
                        {getButtonOptions('presupuesto')}
                        {getAdjuntosCarrusel('presupuesto')}
                    </div>
                </TabPanel>
