...
export default function Convertir(props) { 
    ...
    const [form, setForm] = useState({
        ...
        departamento: data.data ? data.data.id_area : '',
        ...
        proyecto: data.data ? data.data.id_proyecto : '',
        ...
        labelPorveedor:  data.data ? opciones.proyectos.find(proveedor => proveedor.value ==  data.data.id_proyecto).name : 'Proyecto',
        facturaObject: {},

    })
    console.log(data.data)
    // console.log(data.data.id_proyecto)
    console.log(form)
            ...
            if (form.auto1) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        //  console.log('esta')
                        apiPutForm(`requisicion/compras/${form.id}`, newForm, auth.access_token).then(
                            (response) => {
                                ...
                                handleClose('convertir')
                                    ...
                                    if(error.response.status == 400){
                                        ...
                                        Swal.fire({
                                            ...
                                            if (result.isConfirmed) {
                                                apiPutForm(`requisicion/compras/${form.id}/presupuesto`, newForm, auth.access_token).then(
                                                    res => {
                                                    Swal.close()
    ...
    const handleChangeProveedor = (e, value) => {
        if (value && value.name) {
            setForm({
                ...
                labelPorveedor: opciones.proyectos.find(proveedor => proveedor.value == value.value).name
            })
        }
    }
    ...
    const handleSubmit = () => {
        ...
        if (true) {
            ...
            try {
                apiPostForm(`requisicion/compras/${props.data.id}/archivos/s3`, data, auth.access_token)
                    .then(res => {
                        Swal.close()
                        Swal.fire({
                            icon: 'success',
