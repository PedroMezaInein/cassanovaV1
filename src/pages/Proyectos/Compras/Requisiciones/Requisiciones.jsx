...
import Adjuntos from '../../../Administracion/RequisicionCompras/Modales/AdjuntosCompras'
import NuevaRequisicion from '../../../../components/forms/administracion/NuevaRequisicionCompras'
import VerRequisicion from '../../../../components/forms/administracion/VerRequisicion'
import {EdtRequisicionCompra} from '../../../../components/forms/administracion/EdtRequisicionCompra'
// import FiltrarRequisiciones from '../../../../components/forms/administracion/FiltrarRequisiciones'
import Historial from '../../../../components/forms/administracion/HistorialCompras'
import Comentarios from '../../../../components/forms/administracion/ComentariosCompras'
            ...
            aux.push(
                    {
                        ...
                        id:result.id,
                        data: result, 
                        presupuesto: result.presu ? presupuesto(result.presu.nombre+' / '+ result.proyecto.nombre) : '',
        ...
        const handleClick = () => {
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    try {
                        apiPutForm(`requisicion/compras/terminar/${item.id}`, item, userAuth.access_token)
                                .then((response) => {
                ...
                funcion: (item) => {
                    if (!item.data.autorizacion_2 ) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                try {
                                    apiDelete(`requisicion/compras/${item.id}/cancelar`, userAuth.access_token).then(result => {
                                        if (reloadTable) {
                                            reloadTable.reload()
                                        }
                                        Swal.fire({
    ...
    return (
        <>
            ...
            <Modal size="lg" title={"Editar requisición"} show={modal.editar.show} handleClose={handleClose('editar')}>
                <EdtRequisicionCompra data={modal.editar.data} handleClose={handleClose('editar')} reload={reloadTable}/>
            </Modal>

            <Modal size="lg" title={"Adjuntos Requisiciones"} show={modal.adjuntos.show} handleClose={handleClose('adjuntos')}>
