...
import SubirPolitica from "../../components/forms/Comunicados/SubirPolitica";
...
export default function Politicas(){
    ...
    const getInfo = () => {
        try {
            apiGet('comunicado/politicas', auth)
            .then(res => {
                console.log(res)
                let archivos = res.data.politicas.map((file) => {
                    return {
                        id: file.adjuntos[0]? file.adjuntos[0].id : '',
                        name: file.nombre,
                        date: file.Fecha_politica.slice(0,10),
    ...
    return(
        <>
                ...
                <div className='comunicados'>
                    <div>
                        ...
                        <div className="btn-subir">
                                <button onClick={handleShow}> + Nueva politica </button>
                        </div>
                                ...
                                return(
                                    <div key={index} className="comunicado">
                                        ...
                                        <div className="btn-comunicado">
                                            <a href={`${item.url}`} target="_blank" ><button onClick={e=>handleView(viewComunicado)}>Ver Politica {item.name}</button></a>
                                        </div>
                                    </div>  
                                )
            ...
            <Modal size="md"  show={show} onHide={handleClose} centered={true} keyboard={true}>
                <Modal.Header>
                    <Modal.Title>Subir Politica</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <SubirPolitica getinfo={getInfo} handleClose={handleClose}/>
                </Modal.Body>
