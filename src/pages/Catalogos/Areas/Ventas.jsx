...
console.log(key.eventKey)
    ...
    const proccessData = (e) => {
        ...
        for(let key in e.ventas){
            // Imprime el id del ventas
        
            for(let ventas in e.ventas[key]){
                // Imprime el ventas
        
                let auxPartidas = []

                    for(let idpartida in e.ventas[key][ventas]){

                        for(let partida in e.ventas[key][ventas][idpartida]){
                            // Imprime el nombre de cada partida
                            let auxSubpartida = []

                            e.ventas[key][ventas][idpartida][partida].forEach(elemento =>{
                                auxSubpartida.push({
                                    id: elemento.id,
    ...
    return(
        <>
            ...
            <Modal size="lg" title={"Nuevos venta"} show={modal.crear.show} handleClose={()=>handleCloseGastos ('crear')}>
                <ModalAgregar tipo = {key.eventKey} handleClose={()=>handleCloseGastos ('crear')} reload={reloadTable}/>
            </Modal>    

            <Modal size="lg" title={"Editar venta"} show={modal.editarGastos.show} handleClose={()=>handleCloseGastos ('editarGastos')}>
