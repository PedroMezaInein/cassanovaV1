...
class MaterialCliente extends Component {
    ...
    addAdjunto = async () => {
        ...
        if (menuactive === 3) {
            filePath = `empresas/${empresa.id}/tipo-proyecto/${submenuactive}/${levelName}`;
            tipo = submenuactive;
            proyecto = levelName;
        } else {
            filePath = `empresas/${empresa.id}/adjuntos/${opciones_adjuntos[menuactive].slug}`;
        ...
        try {
            let auxPromises = form.adjuntos.adjuntos.files.map(async (file) => {
                // 1) pedir al backend un presigned URL (v2)
                const { data } = await axios.post(
                    `${URL_DEV}v1/s3/presigned-url-v2`,
                    {
                        fileName: file.name,
                ...
                // 2) subir el archivo con PUT a la URL firmada
                await axios.put(data.url, file.file, {
                    headers: { 'Content-Type': file.file.type }
                });

                // 3) devolver info para guardar en la BD
                return { name: file.name, url: data.key };
            });

            const values = await Promise.all(auxPromises);

            // seguir usando tu método actual
            this.addS3FilesAxios(values, tipo, proyecto);
        } catch (err) {
            console.error(err);
            errorAlert('Ocurrió un error al subir adjuntos.');
        }
    };
