...
import { catchErrors, apiGet, apiPostForm, apiPostFormData } from '../functions/api'
    ...
    uploadFilesS3 = async () => {
        ...
        if (mustSendCorreo === 'no' || mustSendCorreo === 'si') {
            ...
            try {
                ...
                formData.append("file", form_orden.adjunto)
                formData.append("orden", form_orden.numero_orden || "")
                formData.append("fechaEvidencia", form_orden.fechaEvidencia || "")
                formData.append("pdfId", form_orden.pdf_id?.id || "")

                // ⚡ Aquí usas directamente tu endpoint correcto
                const { data } = await apiPostFormData(
                    `v3/leads/crm/${lead.id}/orden-compra?_method=PUT`,
                    formData,
                    access_token
                )

                // ✅ Ahora sí, continúa con tu flujo de crear proyecto
                this.createProyectoAxios(
                    {
                        name: form_orden.numero_orden,
                        url: data.url_temporal, // el mergeOrdenCompra ya devuelve url temporal
                        key: data.presupuesto_nuevo?.pivot?.orden_compra
                    },
                    mustSendCorreo
                )
            } catch (err) {
                console.error(err)
                errorAlert("Ocurrió un error al enviar la información")
            }
        } else {
            errorAlert(`Vuelve a intentarlo y selecciona una opción.`)
        }
    }
