...
import CarruselAdjuntos from './CarruselAdjuntosCompras'
...
export default function Adjuntos(props) {
    ...
    const getAdjuntos = () => {
        try {
            apiGet(`requisicion/compras/adjuntos/${props.data.id}`, authUser)
                .then(res => {
    ...
    const handleSubmit = (e) => {
        ...
        if (validate()) {
            ...
            try {
                apiPostForm(`requisicion/compras/${props.data.id}/archivos/s3`, data, authUser)
                    .then(res => {
                        Swal.close()
                        Swal.fire({
                            icon: 'success',
